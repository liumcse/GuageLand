...
const HouseCard = props => (
  <div className="container">
    <div className="card-container">
      <Card className="card">
        <CardContent >
          <div className={(props.morePoints ? "locked" : "")} style={{width: ""}}>
            <img src={props.image} />
          </div>
        </CardContent>
        ...
        <Typography component="p">
          <div className="level-caption">
          {props.morePoints ? "You need " + props.morePoints + " more points to unlock this level" :
...
const houses = [House0, House1, House2, House3, House4, House5, House6, House7, House8,
  House9, House10, House11, House12, House13, House14];
    ...
    <div className="container">
      <Slider {...settings}>
        {range(0, props.currLevel + 1).map(x => {
          return <HouseCard image={houses[x]} level={numToLevel(x)} key={"house" + x}/>
        })}
        {range(props.currLevel + 1, 15).map(x => {
          return <HouseCard image={houses[x]} level={numToLevel(x)} 
            morePoints={(x - props.currLevel) * 200} key={"house" + x}/>
        })}
      </Slider>
      <style jsx>{`
      .container {
